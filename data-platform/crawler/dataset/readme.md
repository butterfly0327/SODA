...
```bash
PYTHONPATH=src python3.12 -m metadata_ingest --source huggingface
PYTHONPATH=src python3.12 -m metadata_ingest --source public_data_portal
PYTHONPATH=src python3.12 -m metadata_ingest --source kaggle
```

### 5-2. 전체 실행

```bash
PYTHONPATH=src python3.12 -m metadata_ingest --source all
```

### 5-3. 처음부터 다시 긁기

```bash
PYTHONPATH=src python3.12 -m metadata_ingest --source figshare --from-scratch
```

### 5-4. 소스별 최대 저장 건수 제한

```bash
PYTHONPATH=src python3.12 -m metadata_ingest --source zenodo --limit 100
```

### 5-5. 지원 소스 목록 확인

```bash
PYTHONPATH=src python3.12 -m metadata_ingest --list-sources
```
...
```bash
# 안전 모드 강제(권장)
PYTHONPATH=src python3.12 -m metadata_ingest --source huggingface --safe

# 안전 모드 + 제한 건수 지정
PYTHONPATH=src python3.12 -m metadata_ingest --source all --safe --limit 30

# 안전 모드 비활성화(요청 간격/지터/배치 pause/소스간 cooldown 비활성)
PYTHONPATH=src python3.12 -m metadata_ingest --source all --no-safe
```
...
### 6-1. `dataset` 컬럼 상세 스펙 (확정)

아래 표는 컬럼 의도와 제약을 정확히 공유하기 위한 기준 명세다.

| 컬럼명 | 타입 | NULL 허용 | 키/제약 사항 | 기본값 | 설명 |
|---|---|---|---|---|---|
| `id` | `BIGSERIAL` | N | PK | auto increment | 데이터셋 내부 식별자 |
| `dataset_source_id` | `SMALLINT` | N | FK -> `dataset_source(id)` | - | 어느 데이터셋 소스에서 수집한 데이터셋인지 |
| `last_ingest_run_id` | `BIGINT` | Y | FK -> `collection_dataset(id)` | `NULL` | 이 row를 마지막으로 갱신한 수집 실행 ID |
| `source_dataset_key` | `TEXT` | N | 복합 UK 대상 | - | 해당 소스가 원래 사용하는 데이터셋 고유 키 |
| `record_hash` | `CHAR(64)` | Y | - | `NULL` | 정규화 결과 기준 내용 해시값 |
| `canonical_url` | `TEXT` | Y | - | `NULL` | 정규화된 대표 URL |
| `landing_url` | `TEXT` | Y | - | `NULL` | 사용자가 실제로 이동할 상세 페이지 URL |
| `title` | `TEXT` | Y | - | `NULL` | 데이터셋 제목 |
| `subtitle` | `TEXT` | Y | - | `NULL` | 데이터셋 부제/보조 제목 |
| `description_short` | `TEXT` | Y | - | `NULL` | 짧은 요약 설명 |
| `description_long` | `TEXT` | Y | - | `NULL` | 긴 상세 설명 |
| `search_text` | `TEXT` | Y | - | `NULL` | 검색 최적화용 통합 텍스트 |
| `publisher_name` | `TEXT` | Y | - | `NULL` | 대표 제공 기관/발행자 이름 |
| `domains` | `TEXT[]` | N | - | `{}` | 도메인/분야 목록 |
| `tasks` | `TEXT[]` | N | - | `{}` | 태스크 목록 |
| `modalities` | `TEXT[]` | N | - | `{}` | 데이터 모달리티 목록 |
| `tags` | `TEXT[]` | N | - | `{}` | 일반 태그/키워드 목록 |
| `languages` | `TEXT[]` | N | - | `{}` | 언어 목록 |
| `license_name` | `TEXT` | Y | - | `NULL` | 라이선스 이름 |
| `license_url` | `TEXT` | Y | - | `NULL` | 라이선스 원문 URL |
| `commercial_use_allowed` | `BOOLEAN` | Y | - | `NULL` | 상업적 사용 가능 여부 |
| `access_type` | `VARCHAR(20)` | Y | `CHECK (access_type IN ('OPEN', 'REGISTERED', 'APPROVAL', 'PAID', 'RESTRICTED', 'UNKNOWN'))` | `NULL` | 대표 접근 유형 |
| `login_required` | `BOOLEAN` | Y | - | `NULL` | 로그인 필요 여부 |
| `approval_required` | `BOOLEAN` | Y | - | `NULL` | 사전 승인 필요 여부 |
| `payment_required` | `BOOLEAN` | Y | - | `NULL` | 유료 여부 |
| `is_restricted` | `BOOLEAN` | Y | - | `NULL` | 접근 제한 상태 여부 |
| `source_created_at` | `TIMESTAMPTZ` | Y | - | `NULL` | 외부 소스 기준 생성 시각 |
| `source_updated_at` | `TIMESTAMPTZ` | Y | - | `NULL` | 외부 소스 기준 마지막 수정 시각 |
| `source_version` | `TEXT` | Y | - | `NULL` | 외부 소스가 제공하는 버전 정보 |
| `row_count` | `BIGINT` | Y | `CHECK (row_count >= 0)` | `NULL` | 행 수/샘플 수 등 대표 규모 지표 |
| `dataset_size_bytes` | `BIGINT` | Y | `CHECK (dataset_size_bytes >= 0)` | `NULL` | 데이터셋 크기(byte) |
| `field_presence_json` | `JSONB` | N | - | `{}` | 주요 필드 존재 여부 요약 |
| `creators_json` | `JSONB` | N | - | `[]` | 저자/기관/연락처 등 상세 구조 |
| `resources_json` | `JSONB` | N | - | `[]` | 파일/다운로드/API/S3 등 리소스 목록 |
| `schema_json` | `JSONB` | N | - | `{}` | 컬럼/feature/split/label 구조 |
| `metrics_json` | `JSONB` | N | - | `{}` | downloads/views/likes 등 원시 메트릭 |
| `extra_json` | `JSONB` | N | - | `{}` | 표준화하지 않은 소스 특수 필드 |
| `raw_json` | `JSONB` | N | - | `{}` | 마지막 수집 원본 또는 정규화 직전 원본 |
| `status` | `VARCHAR(20)` | N | `CHECK (status IN ('ACTIVE', 'INACTIVE', 'DELETED', 'ERROR'))` | `ACTIVE` | 서비스 내 레코드 상태 |
| `last_ingested_at` | `TIMESTAMPTZ` | N | - | `NOW()` | 우리 시스템이 마지막으로 수집한 시각 |
| `created_at` | `TIMESTAMPTZ` | N | - | `NOW()` | 이 `dataset` row 최초 생성 시각 |
| `updated_at` | `TIMESTAMPTZ` | N | - | `NOW()` | 이 `dataset` row 마지막 수정 시각 |
...
```bash
PYTHONPATH=src python3.12 -m metadata_ingest --source public_data_portal --limit 20
PYTHONPATH=src python3.12 -m metadata_ingest --source aihub --limit 20
PYTHONPATH=src python3.12 -m metadata_ingest --source kaggle --limit 20
```
