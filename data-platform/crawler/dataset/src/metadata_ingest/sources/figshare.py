...
class FigshareCollector(BaseDatasetCollector):
        ...
        while True:
            ...
            for item in items:
                ...
                try:
                    ...
                    detail = (
                        self.get_json(f"{self.source.base_url}/articles/{source_key}")
                        if need_detail
                        else item
                    )
    ...
    def _normalize(self, raw: Dict[str, Any]) -> NormalizedDatasetRecord:
        ...
        landing_url = clean_text(
            raw.get("url_public_html") or raw.get("figshare_url") or raw.get("url")
        )
            ...
            resources.append(
                {
                    "id": file.get("id"),
                    "name": clean_text(
                        file.get("name") or file.get("supplied_filename")
                    ),
        ...
        categories = unique_strings(
            cat.get("title") if isinstance(cat, dict) else cat
        ...
        return NormalizedDatasetRecord(
            ...
            license_name=clean_text(
                safe_get(raw, "license", "name") or raw.get("license_name")
            ),
            license_url=clean_text(
                safe_get(raw, "license", "url") or raw.get("license_url")
            ),
            ...
            source_created_at=parse_datetime(
                raw.get("published_date") or raw.get("posted_date")
            ),
            source_updated_at=parse_datetime(raw.get("modified_date")),
            source_version=clean_text(
                raw.get("version") or raw.get("version_id") or raw.get("resource_doi")
            ),
            ...
            metrics_json={
                ...
                "downloads": parse_int(
                    raw.get("downloads") or raw.get("downloads_count")
                ),
