...
class DataGovCollector(BaseDatasetCollector):
    ...
    def _normalize(self, raw: Dict[str, Any]) -> NormalizedDatasetRecord:
        ...
        extras_for_extra = {
            ...
            if key
            not in {"describedBy", "describedByType", "bureauCode", "programCode"}
        }
        ...
        for resource in raw.get("resources") or []:
            if not isinstance(resource, dict):
                continue
            size_bytes = self._extract_resource_size_bytes(resource)
            if size_bytes:
            ...
            resources.append(
                {
                    ...
                    "url_type": clean_text(resource.get("url_type")),
                }
            )

        if total_size <= 0 and bool(getattr(self.settings, "deep_enrichment", False)):
            for resource in resources[:4]:
                if resource.get("size_bytes"):
                    continue
                size_bytes = self._head_content_length(
                    resource.get("url"),
                    resource.get("format"),
    ...
    def _extract_resource_size_bytes(self, resource: Dict[str, Any]) -> Any:
        value = (
            resource.get("size")
            or resource.get("resource_size")
            or resource.get("package_size")
        )
        return parse_bytes(value) or parse_int(value)

    def _head_content_length(
        self, url: Any, resource_format: Any, mime_type: Any
    ) -> Any:
        text = clean_text(url)
        ...
        format_text = clean_text(resource_format)
        mime_text = clean_text(mime_type)
        if not likely_file and not format_text and not mime_text:
            return None
