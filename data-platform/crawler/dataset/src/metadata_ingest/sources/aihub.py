...
_SIZE_PATTERN = re.compile(
    r"(?i)(?:용량|데이터\s*크기|구축량|size|storage|volume)[^0-9]{0,24}(\d+(?:\.\d+)?)\s*(B|KB|MB|GB|TB|PB|KiB|MiB|GiB|TiB|PiB)"
)
...
class AIHubCollector(BaseDatasetCollector):
    ...
    def _normalize(
        ...
        overview_intro = self._extract_overview_intro(soup)
        ...
        dataset_size_bytes = self._extract_dataset_size_bytes(kv, sections, text)
    ...
    def _extract_dataset_size_bytes(
        self, kv: Dict[str, Any], sections: Dict[str, str], page_text: str
    ) -> Optional[int]:
        for key in [
            "구축량",
            "데이터 용량",
        ...
        for section_name in ["데이터 개요", "데이터 통계", "소개"]:
            size_bytes = self._extract_size_from_text(sections.get(section_name))
            if size_bytes:
                return size_bytes

        return self._extract_size_from_text(page_text)
    ...
    def _extract_overview_intro(self, soup: BeautifulSoup) -> Optional[str]:
        for section in soup.select("li[data-accordion-key]"):
            button = section.select_one("h3 button")
            section_name = clean_text(
                button.get_text(" ", strip=True) if button else None
            )
            if not section_name or "데이터 개요" not in section_name:
                continue

            intro = self._extract_intro_from_section(section)
            if intro:
                return intro
        return None

    def _extract_intro_from_section(self, section: BeautifulSoup) -> Optional[str]:
        intro_heading = None
        for heading in section.select("h4"):
            heading_text = clean_text(heading.get_text(" ", strip=True))
            if heading_text and "소개" in heading_text:
                intro_heading = heading
                break

        if intro_heading is None:
            return None

        for sibling in intro_heading.next_siblings:
            ...
            if normalized and not self._is_noisy_description(normalized):
                return normalized
    ...
    def _safe_terms(
        ...
        for term in unique_strings(values):
            if not term:
                continue
            if _JUNK_ROW_RE.match(term) or _JUNK_JSON_RE.match(term):
                continue
