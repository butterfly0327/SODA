...
class HarvardDataverseCollector(BaseDatasetCollector):
    ...
    def iter_records(
        ...
        while True:
            ...
            for item in items:
                ...
                try:
                    detail = None
                    if bool(getattr(self.settings, "deep_enrichment", False)):
    ...
    def _normalize(
        self, search_item: Dict[str, Any], detail_payload: Optional[Dict[str, Any]]
    ) -> NormalizedDatasetRecord:
        detail_candidate = safe_get(detail_payload, "data") or detail_payload or {}
        detail: Dict[str, Any] = (
            detail_candidate if isinstance(detail_candidate, dict) else {}
        )
        latest_candidate = detail.get("latestVersion") or {}
        latest: Dict[str, Any] = (
            latest_candidate if isinstance(latest_candidate, dict) else {}
        )
