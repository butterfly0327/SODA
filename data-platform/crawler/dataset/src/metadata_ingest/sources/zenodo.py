...
class ZenodoCollector(BaseDatasetCollector):
    ...
    def _fetch_records_page(self, page: int) -> Dict[str, Any]:
        ...
        params = {
            "page": page,
            "size": self._effective_page_size,
            "sort": "newest",
        ...
        try:
            ...
            if self._effective_page_size != self.FALLBACK_PAGE_SIZE:
                self._effective_page_size = self.FALLBACK_PAGE_SIZE
                params["size"] = self._effective_page_size
                payload = self.get_json(endpoint, params=params)
            else:
                raise
    ...
    def _normalize(self, raw: Dict[str, Any]) -> NormalizedDatasetRecord:
        ...
        landing_url = clean_text(
            safe_get(raw, "links", "html")
            or raw.get("doi_url")
            or safe_get(raw, "links", "self_html")
        )
            ...
            resources.append(
                {
                    ...
                    "download_url": clean_text(
                        safe_get(file, "links", "self")
                        or safe_get(file, "links", "download")
                    ),
        ...
        access_right = clean_text(
            metadata.get("access_right")
            or metadata.get("accessRight")
            or raw.get("access_right")
        )
        access_type = "OPEN"
        ...
        publisher_name = clean_text(metadata.get("publisher")) or (
            creators[0].get("name") if creators else None
        )
        ...
        return NormalizedDatasetRecord(
            ...
            domains=domains_from_urls(
                [landing_url] + [r.get("download_url") for r in resources]
            ),
            tasks=keywords,
            modalities=guess_modalities_from_text(
                title, description, tags, [r.get("filename") for r in resources]
            ),
            tags=tags,
            languages=unique_strings([metadata.get("language")]),
            license_name=clean_text(
                safe_get(metadata, "license", "id")
                or (
            ...
            license_url=clean_text(
                safe_get(raw, "links", "license")
                or safe_get(metadata, "license", "url")
            ),
            ...
            extra_json={
                ...
                "conceptdoi": clean_text(
                    raw.get("conceptdoi") or metadata.get("conceptdoi")
                ),
