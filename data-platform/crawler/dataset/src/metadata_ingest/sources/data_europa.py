...
class DataEuropaCollector(BaseDatasetCollector):
    ...
    def _normalize(
        ...
        *,
        ...
        landing_url = (
            self._first_url_any(dataset_node, ["landingPage", "url", "uri", "about"])
            or self._first_url_any(search_item, ["landingPage", "url", "uri", "about"])
            or self._http_text(dataset_node.get("@id") or dataset_node.get("id"))
        ...
        for dist in distributions:
            ...
            download_url = self._first_url_any(dist, ["downloadURL", "accessURL"])
            resources.append(
                {
        ...
        if total_size <= 0:
            for resource in resources[:5]:
                size_bytes = self._head_content_length(resource.get("download_url"))
                if not size_bytes:
                    continue
                resource["byte_size"] = size_bytes
                total_size += size_bytes
        ...
        return NormalizedDatasetRecord(
            ...
            source_updated_at=parse_datetime(
                ...
                or self._first_text_any(
                    search_item, ["modified", "updated", "updateDate"]
                )
                or self._nested_text(search_item, ["catalog", "modified"])
            ),
        ...
        for key, value in node.items():
            ...
            for item in ensure_list(value):
                ...
                if isinstance(item, dict):
                    ...
                    if text is None:
                        text = pick_localized_text(
                            item,
                            preferred_langs=("ko", "en", "uk"),
                        )
                else:
                    text = item
    ...
    def _http_text(self, value: Any) -> Optional[str]:
        text = clean_text(value)
        if text and text.startswith("http"):
            return text
        return None

    def _nested_text(self, node: Dict[str, Any], path: List[str]) -> Optional[str]:
        current: Any = node
        for key in path:
            if not isinstance(current, dict):
                return None
            current = current.get(key)
        value = jsonld_value(current)
        if isinstance(value, dict):
            text = jsonld_first(value, ["@value", "value", "label", "name", "@id"])
        else:
            text = value
        return clean_text(text) if text is not None else None
    ...
    def _extract_distributions(
        ...
        for item in ensure_list(value):
            ...
            node_id = clean_text((node or {}).get("@id") or (node or {}).get("id"))
            if node and (not node_id or node_id not in seen_ids):
                results.append(node)
                if node_id:
        ...
        for node in index.values():
            types = self._types(node)
            if not any(t.casefold().endswith("distribution") for t in types):
                continue
            node_id = clean_text(node.get("@id") or node.get("id"))
    ...
    def _head_content_length(self, url: Any) -> Optional[int]:
        text = clean_text(url)
        if not text or not text.startswith("http"):
            return None
        try:
            response = self.request("HEAD", text, retries=1)
        except Exception:
            return None
        content_type = clean_text(response.headers.get("Content-Type")) or ""
        if "text/html" in content_type.casefold():
            return None
        size_bytes = parse_int(response.headers.get("Content-Length"))
