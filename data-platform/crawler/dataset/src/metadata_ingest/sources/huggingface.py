...
class HuggingFaceCollector(BaseDatasetCollector):
    ...
    def _normalize(self, raw: Dict[str, Any]) -> NormalizedDatasetRecord:
        ...
        if dataset_size_bytes is None and bool(
            getattr(self.settings, "deep_enrichment", True)
        ):
            dataset_size_bytes = self._fetch_dataset_size_bytes(source_key)
        ...
        subtitle = clean_text(
            card.get("subtitle") or card.get("tagline") or raw.get("subtitle")
        )
        if subtitle in {title, source_key, source_key.split("/")[-1]}:
            subtitle = None
    ...
    def _fetch_dataset_size_bytes(self, source_key: str) -> Optional[int]:
        if source_key in self._size_cache:
            return self._size_cache[source_key]

        try:
            detail_payload = self.get_json(
                f"https://huggingface.co/api/datasets/{source_key}",
                params={"expand[]": "usedStorage"},
                retries=2,
            )
            detail_size = parse_int(
                safe_get(detail_payload, "usedStorage")
                or safe_get(detail_payload, "used_storage")
            ) or parse_bytes(
                safe_get(detail_payload, "usedStorage")
                or safe_get(detail_payload, "used_storage")
            )
            if detail_size and detail_size > 0:
                self._size_cache[source_key] = detail_size
                return detail_size
        except Exception:
            pass
        ...
        try:
            payload = self.get_json(
                ...
                params={"dataset": quote(source_key, safe="/")},
                retries=2,
            )
        except Exception:
            self._size_cache[source_key] = None
            return None

        candidates = [
            safe_get(payload, "size", "dataset", "num_bytes_original_files"),
            safe_get(payload, "size", "dataset", "num_bytes_parquet_files"),
            safe_get(payload, "num_bytes_original_files"),
            safe_get(payload, "num_bytes_parquet_files"),
        ]

        size_bytes: Optional[int] = None
        for candidate in candidates:
            value = parse_int(candidate) or parse_bytes(candidate)
            if value and value > 0:
                size_bytes = value
                break

        self._size_cache[source_key] = size_bytes
        return size_bytes
